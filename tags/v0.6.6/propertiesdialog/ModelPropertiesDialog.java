 ...
 * 
 *
 * Dialog that presents the Model settings to the user so that they can 
 * set their preferences.  
 * Initially (just now 16th May 2009) almost identical to NodePropertiesDialog.
 * 
 */

public class ModelPropertiesDialog extends PropertiesDialog {

	private EntityViewConfig config;
	
	public ModelPropertiesDialog(YorkBrowser browser, EntityViewConfig configIn){
		super(browser);
	...
	protected void createNodes(DefaultMutableTreeNode top) {
		...
		panelMap.put(MODEL_PREFERENCES, new InfoPanel());
		...
		// Node config
		category = new DefaultMutableTreeNode(new PanelInfo(MODEL_PREFERENCES, panelMap.get(MODEL_PREFERENCES)));
		top.add(category);
